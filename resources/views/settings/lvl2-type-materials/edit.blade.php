        ...
        <div class="mx-auto space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-sm">
                <div class="mx-auto grid w-full py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('lvl2-type-materials.update', $lvl2TypeMaterial->id) }}">
                        @csrf
                        @method('PUT')
                        <!-- Nama -->
                        <div>
                            ...
                            <x-text-input id="nama" class="block mt-1 w-full" type="text" name="nama"
                                :value="old('nama', $lvl2TypeMaterial->nama)" autofocus required />
                        ...
                        <div>
                            ...
                            <x-text-input id="kode" class="block mt-1 w-full" type="text" name="kode"
                                :value="old('kode', $lvl2TypeMaterial->kode)" required />
                            <x-input-error :messages="$errors->get('kode')" class="mt-2" />
                        </div>

                        <!-- Action Buttons -->
                        <div class="flex items-center justify-end mt-6">
                            <a href="{{ route('lvl2-type-materials.index') }}"
