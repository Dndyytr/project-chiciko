                ...
                <div class="mx-auto grid w-full py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between py-5 mb-5">
                        ...
                        <div class="md:mt-0 sm:flex-none w-72">
                            <form action="{{ route('data-expenses.index') }}" method="GET">
...
dark:text-gray-300 dark:focus:border-indigo-700 dark:active:bg-gray-700 dark:active:text-gray-300" />
                        ...
                        <div class="sm:ml-16 sm:mt-0 sm:flex-none">
                            <a type="button" href="{{ route('data-expenses.create') }}"
...
dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <tbody>
                                @php $i = $i ?? 0; @endphp
                                @forelse($dataExpenses as $dataExpense)
                                    <tr
                                        ...
                                        <td class="px-6 py-2 text-center">
                                            {{ $dataExpense->tanggal_nota }}
                                        </td>
                                        <td class="px-6 py-2 text-center">{{ $dataExpense->no_nota }}</td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $dataExpense->kategori }}</td>
                                        <td class="px-6 py-2 text-center">{{ $dataExpense->keterangan }}</td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $dataExpense->harga_satuan }}</td>
                                        <td class="px-6 py-2 text-center">{{ $dataExpense->kuantitas }}</td>
                                        <td class="px-6 py-2 text-center">{{ $dataExpense->kredit }}</td>
                                        <td class="px-6 py-2 text-center">
                                            <form action="{{ route('data-expenses.destroy', $dataExpense->id) }}"
                                                method="POST">
                                                <a href="{{ route('data-expenses.edit', $dataExpense->id) }}"
