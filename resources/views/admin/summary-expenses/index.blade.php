                ...
                <div class="mx-auto grid w-full py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between py-5 mb-5">
                        ...
                        <div class="md:mt-0 sm:flex-none w-72">
                            <form action="{{ route('summary-expenses.index') }}" method="GET">
...
dark:text-gray-300 dark:focus:border-indigo-700 dark:active:bg-gray-700 dark:active:text-gray-300" />
                        ...
                        <div class="sm:ml-16 sm:mt-0 sm:flex-none">
                            <a type="button" href="{{ route('summary-expenses.create') }}"
...
dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <th scope="col" class="px-6 py-3 text-center">
                                        <span>Kategori dan Keuangan</span>
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-center">
                            ...
                            <tbody>
                                @forelse($summaryExpenses as $summaryExpense)
                                    <tr
...
text-center">
                                        ...
                                        <td class="px-6 py-2 text-center">
                                            @if ($summaryExpense->tanggal_mulai && $summaryExpense->tanggal_akhir)
                                                {{ format_tanggal_id($summaryExpense->tanggal_mulai, false, true) }} -
                                                {{ format_tanggal_id($summaryExpense->tanggal_akhir, false, true) }}
                                            @elseif ($summaryExpense->tanggal_mulai && !$summaryExpense->tanggal_akhir)
                                                Dari
                                                {{ format_tanggal_id($summaryExpense->tanggal_mulai, false, true) }}
                                            @elseif (!$summaryExpense->tanggal_mulai && $summaryExpense->tanggal_akhir)
                                                Sampai
                                        ...
                                        <td class="px-6 py-2 text-center">
                                            @foreach ($summaryExpense->summaryExpenseDetails as $detail)
                                                <span>{{ $detail->kategori }}
                                                    {{ $detail->total_uang_keluar }}<br /></span>
                                            @endforeach
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $summaryExpense->total_keseluruhan }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            <form action="{{ route('summary-expenses.destroy', $summaryExpense->id) }}"
                                                method="POST">
                                                <div class="flex">
                                                    @csrf
                                                    @method('DELETE')
