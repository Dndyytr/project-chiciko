<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Pembelian Berdasarkan Jumlah Roll Kain') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="mx-auto">
                ...
                <div class="mx-auto grid w-full py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between py-5 mb-5">
                        ...
                        <div class="md:mt-0 sm:flex-none w-72">
                            <form action="{{ route('purchase-based-rolls.index') }}" method="GET">
...
dark:text-gray-300 dark:focus:border-indigo-700 dark:active:bg-gray-700 dark:active:text-gray-300" />
                        ...
                        <div class="sm:ml-16 sm:mt-0 sm:flex-none">
                            <a type="button" href="{{ route('purchase-based-rolls.create') }}"
...
dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <tbody>
                                @forelse($purchaseBasedRolls as $purchaseBasedRoll)
                                    <tr
...
text-center">
                                        ...
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->kode_barcode }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->nama_barang }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->jenis_kain }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->warna }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->qty_roll }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->yard_per_roll }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->kg_per_roll }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->jumlah_roll_satuan }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->total_harga }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $purchaseBasedRoll->harga_per_satuan }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            <form
                                                action="{{ route('purchase-based-rolls.destroy', $purchaseBasedRoll->id) }}"
                                                method="POST">
                                                <div class="flex">
                                                    <a href="{{ route('purchase-based-rolls.edit', $purchaseBasedRoll->id) }}"
