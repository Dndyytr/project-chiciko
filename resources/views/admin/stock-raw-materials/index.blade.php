<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Stock Persediaan Bahan Baku') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="mx-auto">
                ...
                <div class="mx-auto grid w-full py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <div class="flex items-center justify-between py-5 mb-5">
                        ...
                        <div class="md:mt-0 sm:flex-none w-72">
                            <form action="{{ route('stock-raw-materials.index') }}" method="GET">
...
dark:text-gray-300 dark:focus:border-indigo-700 dark:active:bg-gray-700 dark:active:text-gray-300" />
                        ...
                        <div class="sm:ml-16 sm:mt-0 sm:flex-none">
                            <a type="button" href="{{ route('stock-raw-materials.create') }}"
...
dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <th scope="col" class="px-6 py-3 text-center">
                                        <span>Keluar Roll</span>
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-center">
                                        <span>Keluar Yard</span>
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-center">
                            ...
                            <tbody>
                                @php $i = $i ?? 0; @endphp
                                @forelse($stockRawMaterials as $stockRawMaterial)
                                    <tr
...
dark:text-white text-center">
                                        ...
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->nama_item }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->keluar_roll }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->keluar_yard }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->stock_akhir }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ format_number_id($stockRawMaterial->sisa_roll) }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->sisa_yard }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->total_harga }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            {{ $stockRawMaterial->harga_per_satuan }}
                                        </td>
                                        <td class="px-6 py-2 text-center">
                                            <form
                                                action="{{ route('stock-raw-materials.destroy', $stockRawMaterial->id) }}"
                                                method="POST">
                                                <div class="flex">
                                                    @csrf
                                                    @method('DELETE')
