...
class Lvl3TypeMaterialObserver
{
     ...
     */
    private function updateLargeDataset($query, Lvl3TypeMaterial $parent): void
    {
        $updates = [];

        // Siapkan field yang akan di-update
        if ($parent->isDirty('nama')) {
            $updates['text_lvl3_jb'] = $parent->nama;
        }
     ...
     */
    private function updateSmallDataset($subMaterials, Lvl3TypeMaterial $parent): void
    {
        foreach ($subMaterials as $sub) {
            $changed = false;

            // Update text_lvl3_jb jika nama berubah
            if ($parent->isDirty('nama')) {
                $sub->text_lvl3_jb = $parent->nama;
                $changed = true;
            }
