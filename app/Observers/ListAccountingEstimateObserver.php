...
class ListAccountingEstimateObserver
{
     ...
     */
    private function updateLargeDataset($query, ListAccountingEstimate $parent): void
    {
        $updates = [];

        // Siapkan field yang akan di-update
        if ($parent->isDirty('nama')) {
            $updates['text_jp'] = $parent->nama;
        }
     ...
     */
    private function updateSmallDataset($subMaterials, ListAccountingEstimate $parent): void
    {
        foreach ($subMaterials as $sub) {
            $changed = false;

            // Update text_jp jika nama berubah
            if ($parent->isDirty('nama')) {
                $sub->text_jp = $parent->nama;
                $changed = true;
            }
