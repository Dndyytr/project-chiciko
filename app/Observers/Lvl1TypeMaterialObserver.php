...
class Lvl1TypeMaterialObserver
{
     ...
     */
    private function updateLargeDataset($query, Lvl1TypeMaterial $parent): void
    {
        $updates = [];

        // Siapkan field yang akan di-update
        if ($parent->isDirty('nama')) {
            $updates['text_lvl1_jb'] = $parent->nama;
        }
     ...
     */
    private function updateSmallDataset($subMaterials, Lvl1TypeMaterial $parent): void
    {
        foreach ($subMaterials as $sub) {
            $changed = false;

            // Update text_lvl1_jb jika nama berubah
            if ($parent->isDirty('nama')) {
                $sub->text_lvl1_jb = $parent->nama;
                $changed = true;
            }
