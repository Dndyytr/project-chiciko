...
class PurchaseBasedYardController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $purchaseBasedYards = PurchaseBasedYard::when($search, function ($purchaseBasedYards) {
            // untuk melakukan pencarian data jika $search ada nilainya
            $purchaseBasedYards = $purchaseBasedYards->where('kode_barcode', 'like', '%' . request()->search . '%')
        ...
        return view('admin.purchase-based-yards.index', compact('purchaseBasedYards'))
            ->with('i', ($page - 1) * $entries); // mengirim $purchaseBasedYards ke view list-accounting-estimates.index, dan i sebagai nomor urut
    }
    ...
    {
        return view('admin.purchase-based-yards.create');
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            $purchaseBasedYard = new PurchaseBasedYard([
            ...
            $purchaseBasedYard->save();

            return redirect()->route('purchase-based-yards.index')->with('success', 'Purchase Based Yard ' . $purchaseBasedYard->kode_barcode . ' created successfully.');
        } catch (\Throwable $th) {
            //throw $th;
            return redirect()->route('purchase-based-yards.create')->with('error', $th->getMessage());
        }
    }
     ...
     */
    public function show(PurchaseBasedYard $purchaseBasedYard)
    {
        //
    }
     ...
     */
    public function edit(PurchaseBasedYard $purchaseBasedYard)
    {
        return view('admin.purchase-based-yards.edit', compact('purchaseBasedYard'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, PurchaseBasedYard $purchaseBasedYard)
    {
        try {
            $request->validate([
                'incoming_raw_materials_id' => 'required|exists:incoming_raw_materials,id',
            ...
            $purchaseBasedYard->update($input);

            return redirect()->route('purchase-based-yards.index')->with('success', 'Purchase Based Yard ' . $purchaseBasedYard->kode_barcode . ' updated successfully.');
        } catch (\Throwable $th) {
            //throw $th;
            return redirect()->route('purchase-based-yards.edit', $purchaseBasedYard->id)->with('error', $th->getMessage());
        }
    }
     ...
     */
    public function destroy(PurchaseBasedYard $purchaseBasedYard)
    {
        if ($purchaseBasedYard) {
            $purchaseBasedYard->delete();
            return redirect()->route('purchase-based-yards.index')->with('success', 'Purchase Based Yard ' . $purchaseBasedYard->kode_barcode . ' deleted successfully.');
        } else {
            return back()->with('error', 'Purchase Based Yard not found!');
        }
    }
    ...
    public function getYards(Request $request)
    {
        ...
        $cacheKey = "data_yards_" . ($search ? md5($search) : 'full');
